...
spec:
  ...
  it('(OLS-743,anpicker,Lightspeed) Test OpenShift Lightspeed with pod', {tags: ['e2e','admin', '@smoke']}, () => {
    ...
    cy.get('[data-test-rows="resource-row"]', { timeout: 30 * 1000 })
      .should('have.length.at.least', 1);

    cy.get('[data-test-rows="resource-row"]:first-of-type [id="name"] a', { timeout: 30 * 1000 })
      .click();
  ...
  it('(OLS-745,anpicker,Lightspeed) Test attaching YAML', {tags: ['e2e','admin', '@smoke']}, () => {
    // Test attaching pod YAML
    cy.get(attachMenu)
      .find('li:first-of-type button')
      .contains('YAML')
      .click();
    cy.get(attachments)
  ...
  it('(OLS-746,anpicker,Lightspeed) Test attaching events', {tags: ['e2e','admin', '@smoke']}, () => {
    cy.get(attachMenuButton).click();
    cy.get(attachMenu)
      .find('button')
      .contains('Events')
  ...
  it('(OLS-747,anpicker,Lightspeed) Test attaching logs', {tags: ['e2e','admin', '@smoke']}, () => {
    cy.get(attachMenuButton).click();
    cy.get(attachMenu)
      .find('button')
      .contains('Logs')
