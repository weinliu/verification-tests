...
var _ = g.Describe("[sig-networking] SDN", func() {
	...
	g.It("ROSA-OSD_CCS-ConnectedOnly-Author:huirwang-Medium-47272-Pods will not be affected by the egressIP set on other netnamespace. [Serial]", func() {
	...
	// author: huirwang@redhat.com
	g.It("ROSA-OSD_CCS-ConnectedOnly-Author:huirwang-Medium-47164-Medium-47025-Be able to update egressip object,The pods removed matched labels will not use EgressIP [Serial]", func() {
	...
	// author: huirwang@redhat.com
	g.It("ROSA-OSD_CCS-ConnectedOnly-Author:huirwang-Medium-47030-An EgressIP object can not have multiple egress IP assignments on the same node. [Serial]", func() {

		buildPruningBaseDir := exutil.FixturePath("testdata", "networking")
		egressIPTemplate := filepath.Join(buildPruningBaseDir, "egressip-config1-template.yaml")

		g.By("1. Get two worker nodes with same subnets")
		nodeList, err := e2enode.GetReadySchedulableNodes(context.TODO(), oc.KubeFramework().ClientSet)
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	// author: huirwang@redhat.com
	g.It("ROSA-OSD_CCS-ConnectedOnly-Author:huirwang-Medium-47028-After remove EgressIP node tag, EgressIP will failover to other availabel egress nodes. [Serial]", func() {
		buildPruningBaseDir := exutil.FixturePath("testdata", "networking")
		pingPodTemplate := filepath.Join(buildPruningBaseDir, "ping-for-pod-template.yaml")
		egressIPTemplate := filepath.Join(buildPruningBaseDir, "egressip-config1-template.yaml")
	...
	// author: huirwang@redhat.com
	g.It("ROSA-OSD_CCS-ConnectedOnly-Author:huirwang-Longduration-NonPreRelease-High-47031-After reboot egress node EgressIP still work.  [Disruptive]", func() {
		buildPruningBaseDir := exutil.FixturePath("testdata", "networking")
	...
	// author: huirwang@redhat.com
	g.It("ROSA-OSD_CCS-ConnectedOnly-Author:huirwang-Longduration-NonPreRelease-Critical-47032-High-47034-Traffic is load balanced between egress nodes,multiple EgressIP objects can have multiple egress IPs.[Serial]", func() {
		buildPruningBaseDir := exutil.FixturePath("testdata", "networking")
		pingPodTemplate := filepath.Join(buildPruningBaseDir, "ping-for-pod-template.yaml")
		egressIPTemplate := filepath.Join(buildPruningBaseDir, "egressip-config1-template.yaml")
	...
	// author: huirwang@redhat.com
	g.It("ROSA-OSD_CCS-ConnectedOnly-Author:huirwang-High-47019-High-47023-EgressIP works well with networkpolicy and egressFirewall. [Serial]", func() {
	...
	// author: huirwang@redhat.com
	g.It("ROSA-OSD_CCS-Author:huirwang-Medium-47018-Medium-47017-Multiple projects use same EgressIP,EgressIP works for all pods in the namespace with matched namespaceSelector. [Serial]", func() {
		buildPruningBaseDir := exutil.FixturePath("testdata", "networking")
		testPodFile := filepath.Join(buildPruningBaseDir, "testpod.yaml")
		egressIPTemplate := filepath.Join(buildPruningBaseDir, "egressip-config1-template.yaml")
		...
		g.By("Create test pods in third namespace  \n")
	...
	g.It("NonHyperShiftHOST-ConnectedOnly-Author:huirwang-Longduration-NonPreRelease-Medium-47033-If an egress node is NotReady traffic is still load balanced between available egress nodes. [Disruptive]", func() {
		...
		if !acceptedPlatform || (isSpecialSTSCluster && strings.Contains(platform, "aws")) || (isSpecialSTSCluster && strings.Contains(platform, "gcp")) {
			g.Skip("This test case should be run on AWS, GCP, OSP, vSphere and IPI BM, skip for other platforms or special AWS/GCP STS cluster!!")
		}
	...
	g.It("NonHyperShiftHOST-ConnectedOnly-NonPreRelease-PstChkUpgrade-Author:jechen-High-56875-OVN egressIP should still be functional post upgrade. [Disruptive]", func() {
		...
		g.By("2. Check source IP from the test pod of the namespace is the assigned egress IP address")
		var dstHost, primaryInf string
		var infErr, snifErr error
		...
		g.By("4. Unlabel the first egress node to cause egressIP failover to the second egress node")
		e2enode.RemoveLabelOffNode(oc.KubeFramework().ClientSet, egressNode1, egressNodeLabel)
		...
		g.By("7. Delete egressIP object, verify egressip is cleared\n")
		...
		g.By("8. Verify node's IP will be used as source IP from the namespace and test pod\n")
...
var _ = g.Describe("[sig-networking] SDN OVN EgressIP Basic", func() {
	...
	g.It("NonHyperShiftHOST-NonPreRelease-ConnectedOnly-Author:jechen-High-54045-EgressIP health check through monitoring port over GRPC on OCP OVN cluster. [Disruptive]", func() {
		...
		g.By("8. Verify egressIP is not assigned after blocking iptable rule on port 9170 is added.\n")
		egressIPMaps1 = getAssignedEIPInEIPObject(oc, egressip1.name)
		o.Expect(len(egressIPMaps1)).Should(o.Equal(0))
...
var _ = g.Describe("[sig-networking] SDN OVN EgressIP", func() {
	...
	// author: huirwang@redhat.com
	g.It("ROSA-OSD_CCS-ConnectedOnly-Author:huirwang-High-47163-High-47026-Deleting EgressIP object and recreating it works,EgressIP was removed after delete egressIP object. [Serial]", func() {
		buildPruningBaseDir := exutil.FixturePath("testdata", "networking")
		pingPodTemplate := filepath.Join(buildPruningBaseDir, "ping-for-pod-template.yaml")
		egressIPTemplate := filepath.Join(buildPruningBaseDir, "egressip-config1-template.yaml")
	...
	g.It("NonHyperShiftHOST-Longduration-NonPreRelease-ConnectedOnly-Author:jechen-High-54647-No stale or duplicated SNAT on gateway router after egressIP failover to new egress node. [Disruptive]", func() {
		...
		g.By("5.1 Create a statefulSet Hello pod in the namespace, apply pod label to it. ")
		...
		g.By("5.2 Create a completed pod in the namespace, apply pod label to it. ")
		...
		g.By("6. Check SNATs of stateful pod and completed pod on the egressNode before rebooting it.\n")
		...
		g.By("10. Check SNAT on the second egressNode\n")
		...
		g.By("10.1 There should be the IP of the newly created statefulState hello pod, not the IP of old hello pod.\n")
		o.Expect(snatIP).Should(o.Equal(newHelloPodIPv4))
		o.Expect(snatIP).ShouldNot(o.Equal(helloPodIPv4))

		g.By("10.2 There should be no SNAT for old or new completed pod's IP.\n")
