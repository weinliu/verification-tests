...
var _ = g.Describe("[sig-networking] SDN udn EgressIP genetic", func() {
	...
	// author: jechen@redhat.com
	g.It("Author:jechen-NonHyperShiftHOST-ConnectedOnly-NonPreRelease-High-78663-Pods on default network and UDNs can access k8s service when its node is egressIP node [Serial]", func() {
		var (
			buildPruningBaseDir = exutil.FixturePath("testdata", "networking")
			egressIP1Template   = filepath.Join(buildPruningBaseDir, "egressip-config1-template.yaml")
			pingPodNodeTemplate = filepath.Join(buildPruningBaseDir, "ping-for-pod-specific-node-template.yaml")
		...
		exutil.By("2.1 Get a namespace for default network, Create four more namespaces for two overlapping layer3 UDNs and overlapping layer2 UDNs")
		ns1 := oc.Namespace()
