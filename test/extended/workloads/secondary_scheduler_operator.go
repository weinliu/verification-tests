...
var _ = g.Describe("[sig-scheduling] Workloads The Descheduler Operator automates pod evictions using different profiles", func() {
	...
	g.BeforeEach(func() {
		...
		sub = ssoSubscription{
			...
			startingCSV: "secondaryscheduleroperator.v1.3.1",
		...
		sub.skipMissingCatalogsources(getOCPerKubeConf(oc, guestClusterKubeconfig))
	...
	g.It("Author:knarra-NonHyperShiftHOST-ROSA-OSD_CCS-ARO-Critical-48916-Critical-48917-Install seconday scheduler operator via a deployment & verify user is able to schedule pod using secondary-scheduler [Serial]", func() {
		...
		o.Expect(strings.Contains(ssCsvOutput, "secondaryscheduleroperator.v1.3.1")).To(o.BeTrue())
		...
		if strings.Contains(rebasedVersion, kuberVersion) || strings.Contains(rebasedVersion, "1.29") {
			e2e.Logf("SSO operator has been rebased with latest kubernetes")
		} else {
	...
	// author: knarra@redhat.com
	g.It("Author:knarra-HyperShiftMGMT-ROSA-OSD_CCS-ARO-Critical-74405-Install seconday scheduler operator via a deployment [Serial]", func() {
		secondarySchedPodT := filepath.Join(buildPruningBaseDir, "deployPodWithScheduler.yaml")
		schedPodWithOutSchedT := filepath.Join(buildPruningBaseDir, "deployPodWithOutScheduler.yaml")
