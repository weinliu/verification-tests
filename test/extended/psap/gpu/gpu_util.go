...
func createMachinesetbyInstanceType(oc *exutil.CLI, machinesetName string, instanceType string) {
	...
	ocGetMachineset, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("machineset", "-n", "openshift-machine-api", "-oname").Output()
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	machinesetYamlOutput, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("machineset", firstMachinesetName, "-n", "openshift-machine-api", "-oyaml").Output()
	o.Expect(err).NotTo(o.HaveOccurred())
