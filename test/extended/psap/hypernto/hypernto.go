...
var _ = g.Describe("[sig-node] PSAP should", func() {
	...
	g.It("HyperShiftMGMT-Author:liqcui-Medium-53875-NTO Support profile that have the same name with tuned on hypershift [Disruptive]", func() {
		...
		exutil.ApplyOperatorResourceByYaml(oc, "", tunedWithSameProfileName)
		...
		nodePoolName := getNodePoolNamebyHosteClusterName(oc, guestClusterName)
		...
		renderCheck, err := getTunedRenderInHostedClusterInHostedCluster(oc, ntoNamespace)
		o.Expect(err).NotTo(o.HaveOccurred())
