...
// NewEnablePasswdSSHInPoolMachineConfig returns a MC that will enable the ssh password authentication in the pool
func NewEnablePasswdSSHInPoolMachineConfig(mcp *MachineConfigPool) (*MachineConfig, error) {
	sshdConfigPath := "/etc/ssh/sshd_config"

	mcpNodes, err := mcp.GetNodes()
	if err != nil {
		logger.Errorf("Error getting the nodes that belong to MCP %s", mcp.GetName())
		return nil, err
	}

	if len(mcpNodes) == 0 {
		logger.Errorf("0 nodes configured in MCP %s", mcp.GetName())
		return nil, fmt.Errorf("There is no pool configured in MCP %s", mcp.GetName())
	}
	firstNode := mcpNodes[0]

	logger.Infof("Get current sshd_config content")
	remoteSSHD := NewRemoteFile(firstNode, sshdConfigPath)
	err = remoteSSHD.Fetch()
	if err != nil {
		logger.Errorf("Error getting the content of %s file in node %s", sshdConfigPath, firstNode.GetName())
		return nil, err
	}

	origSSHDContent := remoteSSHD.GetTextContent()
	logger.Debugf("Original %s content:\n%s", sshdConfigPath, origSSHDContent)

	regc := regexp.MustCompile(`PasswordAuthentication +no`)
	modifiedSSHDContent := regc.ReplaceAllString(origSSHDContent, "PasswordAuthentication yes")
	logger.Debugf("Modified %s content:\n%s", sshdConfigPath, modifiedSSHDContent)

	logger.Infof("Create a MC to enable the passwords in sshd")
	fileConfig := getURLEncodedFileConfig(sshdConfigPath, modifiedSSHDContent, "0600")
	mc := NewMachineConfig(mcp.oc.AsAdmin(), "enable-passwd-ssh-"+mcp.GetName(), mcp.GetName())
