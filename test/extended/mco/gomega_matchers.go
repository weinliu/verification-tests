...
// struct implementing gomega matcher interface for RemoteFile content
type remoteFileContentMatcher struct {
	textContent     string
	expected        interface{}
	expectedMatcher types.GomegaMatcher
}
...
func (matcher *remoteFileContentMatcher) Match(actual interface{}) (success bool, err error) {
	...
	matcher.textContent = remoteFile.GetTextContent()
	...
	return matcher.expectedMatcher.Match(matcher.textContent)
}

// FailureMessage returns the message when testing `Should` case and `Match` returned false
func (matcher *remoteFileContentMatcher) FailureMessage(actual interface{}) (message string) {
	// The type was already validated in Match, we can safely ignore the error
	remoteFile, ok := actual.(RemoteFile)
	if !ok {
	...
	message = fmt.Sprintf("%s\n, the matcher was not satisfied by the content of the remote file %s in node %s",
		matcher.expectedMatcher.FailureMessage(matcher.textContent), remoteFile.GetFullPath(), remoteFile.node.GetName())
	return message
}

// FailureMessage returns the message when testing `ShouldNot` case and `Match` returned true
func (matcher *remoteFileContentMatcher) NegatedFailureMessage(actual interface{}) (message string) {
	// The type was already validated in Match, we can safely ignore the error
	remoteFile, ok := actual.(RemoteFile)
	if !ok {
		remoteFilePtr, _ := actual.(*RemoteFile)
	...
	message = fmt.Sprintf("%s\n, the matcher was satisfied (but it should NOT) by the content of the remote file %s in node %s",
		matcher.expectedMatcher.NegatedFailureMessage(matcher.textContent), remoteFile.GetFullPath(), remoteFile.node.GetName())
...
func HaveContent(expected interface{}) types.GomegaMatcher {
	return &remoteFileContentMatcher{expected: expected}
}
