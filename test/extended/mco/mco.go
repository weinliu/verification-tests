...
`
	...
	g.It("Author:ptalgulk-NonHyperShiftHOST-NonPreRelease-Longduration-Low-74048-Override CRIO restart action to CRIO reload action through MCD[Disruptive]", func() {
		var (
			createdCustomPoolName = "infra"
			fileContent           = "MCO test file permissions"
			filePath1             = "/etc/pki/ca-trust/source/anchors/openshift-config-user-ca-bundle.crt"
			filePath2             = "/etc/machine-config-daemon/no-reboot/containers-gpg.pub"
			mcName                = "test-file"
			// help to check MC applied is vailed and
			// could help to check crio is restrted or not when new MC is been applied with required config
			behaviourValidatorApply = UpdateBehaviourValidator{
				RebootNodesShouldBeSkipped: true,
				DrainNodesShoulBeSkipped:   true,
				RestartCrioShouldBeSkipped: false,
				Checkers: []Checker{
					RemoteFileChecker{
						FileFullPath: filePath1,
						Matcher:      HaveContent(fileContent),
						ErrorMsg:     fmt.Sprintf("The content of file %s is not as expected ", filePath1),
						Desc:         fmt.Sprintf("To verfiy the content of %s is properly applied", filePath1),
					},
					RemoteFileChecker{
						FileFullPath: filePath2,
						Matcher:      HaveContent(fileContent),
						ErrorMsg:     fmt.Sprintf("The content of file %s is not as expected ", filePath2),
						Desc:         fmt.Sprintf("To verfiy the content of %s is properly applied", filePath2),
					},
				},
			}
		)
		...
		newMcp, err := CreateCustomMCP(oc.AsAdmin(), createdCustomPoolName, 1)
		o.Expect(err).NotTo(o.HaveOccurred(), "Could not create a new custom MCP")
		node := newMcp.GetNodesOrFail()[0]
		logger.Infof("OK\n")

		defer func() {
			exutil.By("Check the file content is deleted")
			o.Expect(NewRemoteFile(node, filePath1).GetTextContent()).NotTo(o.Equal(fileContent),
				"%s should not be empty file", filePath1)
			o.Expect(NewRemoteFile(node, filePath2)).NotTo(Exist(),
				"The file %s should NOT Exists", filePath2, fileContent)

			logger.Infof("OK\n")
		}()

		behaviourValidatorApply.Initialize(newMcp, nil)

		exutil.By("create a new MC")
		mc := NewMachineConfig(oc.AsAdmin(), mcName, newMcp.GetName())

		fileConfig1 := getURLEncodedFileConfig(filePath1, fileContent, "")
		fileConfig2 := getURLEncodedFileConfig(filePath2, fileContent, "")
		mc.parameters = []string{fmt.Sprintf("FILES=[%s, %s]", fileConfig1, fileConfig2)}
