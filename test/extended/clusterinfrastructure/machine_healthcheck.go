...
var _ = g.Describe("[sig-cluster-lifecycle] Cluster_Infrastructure", func() {
	...
	g.It("Author:huliu-Low-45343-[MHC] - nodeStartupTimeout in MachineHealthCheck should revert back to default [Flaky]", func() {
		...
		nodeStartupTimeoutBeforeUpdate, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("machinehealthchecks.machine.openshift.io", "machine-api-termination-handler", "-o=jsonpath={.spec.nodeStartupTimeout}", "-n", machineAPINamespace).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
		e2e.Logf("before update --- nodeStartupTimeout: " + nodeStartupTimeoutBeforeUpdate)

		g.By("Update nodeStartupTimeout to 30m")
		err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("machinehealthchecks.machine.openshift.io/machine-api-termination-handler", "-n", machineAPINamespace, "-p", `{"spec":{"nodeStartupTimeout":"30m"}}`, "--type=merge").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		g.By("Wait nodeStartupTimeout revert back to default itself")
		err = wait.Poll(30*time.Second, 360*time.Second, func() (bool, error) {
			output, _ := oc.AsAdmin().WithoutNamespace().Run("get").Args("machinehealthchecks.machine.openshift.io", "machine-api-termination-handler", "-o=jsonpath={.spec.nodeStartupTimeout}", "-n", machineAPINamespace).Output()
			if output == "30m" {
		...
		nodeStartupTimeoutAfterUpdate, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("machinehealthchecks.machine.openshift.io", "machine-api-termination-handler", "-o=jsonpath={.spec.nodeStartupTimeout}", "-n", machineAPINamespace).Output()
		o.Expect(err).NotTo(o.HaveOccurred())
