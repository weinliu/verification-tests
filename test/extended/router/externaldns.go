...
var _ = g.Describe("[sig-network-edge] Network_Edge should", func() {
	...
	var (
		...
		createRecordsLog  = "Desired change: CREATE external-dns-canary-openshift-ingress-canary"
		deleteRecordsLog  = "Desired change: DELETE external-dns-canary-openshift-ingress-canary"
	)
	...
	g.It("ConnectedOnly-Author:hongli-High-48138-Support External DNS on AWS platform", func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		ensureLogsContainString(oc, operandNamespace, operandLabel, createRecordsLog)
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		ensureLogsContainString(oc, operandNamespace, operandLabel, deleteRecordsLog)
	})
})
