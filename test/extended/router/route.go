...
var _ = g.Describe("[sig-network-edge] Network_Edge should", func() {
	...
	g.It("HyperShiftGUEST-ROSA-OSD_CCS-ARO-Author:aiyengar-Medium-42230-route can be configured to whitelist more than 61 ips/CIDRs", func() {
		var (
	...
	g.It("HyperShiftGUEST-ROSA-OSD_CCS-ARO-Author:mjoseph-High-45399-ingress controller continue to function normally with unexpected high timeout value", func() {
		var (
			buildPruningBaseDir = exutil.FixturePath("testdata", "router")
			output              string
			testPodSvc          = filepath.Join(buildPruningBaseDir, "web-server-rc.yaml")
	...
	// author: mjoseph@redhat.com
	g.It("HyperShiftGUEST-ROSA-OSD_CCS-ARO-Author:mjoseph-High-49802-HTTPS redirect happens even if there is a more specific http-only", func() {
	...
	// author: mjoseph@redhat.com
	g.It("HyperShiftGUEST-Longduration-Author:mjoseph-Critical-53696-Route status should updates accordingly when ingress routes cleaned up [Disruptive]", func() {
		var (
			buildPruningBaseDir = exutil.FixturePath("testdata", "router")
